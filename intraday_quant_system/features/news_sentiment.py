...
class NewsSentimentEngine:
    ...
    def __init__(self, hours_lookback: int = 48):
        ...
        if pipeline:
            ...
            logger.warning(
                "transformers not installed. Returning neutral sentiment 0.0."
            )
            self.nlp = None
    ...
    def fetch_macro_sentiment(self) -> float:
        ...
        try:
            ...
            logger.info(
                f"Macro Systemic Sentiment: {self.macro_sentiment:.2f} based on {count} events."
            )
            return self.macro_sentiment
        except Exception as e:
    ...
    def compute_sentiment(self, symbol: str) -> float:
        ...
        try:
            ...
            logger.debug(
                f"[{symbol}] FinBERT Blended Sentiment: {blended_score:.2f} (Micro: {micro_score:.2f}, Macro: {macro_score:.2f})"
            )

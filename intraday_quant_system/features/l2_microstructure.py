...
class L2Microstructure:
    ...
    @staticmethod
    def quote_slope(
        bid_prices: list, ask_prices: list, bid_vols: list, ask_vols: list
    ) -> float:
        """Slope of liquidity on bid vs ask side"""
        if len(bid_prices) < 2 or len(ask_prices) < 2:
            return 0.0
        bid_slope = (bid_vols[-1] - bid_vols[0]) / (
            bid_prices[0] - bid_prices[-1] + 1e-6
        )
        ask_slope = (ask_vols[-1] - ask_vols[0]) / (
            ask_prices[-1] - ask_prices[0] + 1e-6
        )
    ...
    def vpin_proxy(df_ticks: pd.DataFrame, bucket_vol: int = 50000) -> float:
        ...
        if (
            df_ticks.empty
            or "volume" not in df_ticks.columns
            or "aggressor_side" not in df_ticks.columns
        ):
            return 0.0
        ...
        vpin = abs(buckets["buy_vol"] - buckets["sell_vol"]).sum() / (

...
def adx(df: pd.DataFrame, period: int = 14) -> pd.Series:
    ...
    # True Range - already uses shift(1) for tr2/tr3
    tr1 = high - low
    ...
    # Shift TR and DM by 1 to prevent lookahead bias
    atr = tr.shift(1).ewm(alpha=1 / period, adjust=False).mean()
    plus_di = 100 * (plus_dm.shift(1).ewm(alpha=1 / period, adjust=False).mean() / atr)
    minus_di = 100 * (
        minus_dm.shift(1).ewm(alpha=1 / period, adjust=False).mean() / atr
    )
...
def momentum(df: pd.DataFrame, period: int = 10) -> pd.Series:
    ...
    if "close" not in df.columns:
        return pd.Series(index=df.index, dtype=float)
    # Shift close by 1 to prevent lookahead bias
    return df["close"].shift(1).pct_change(periods=period)

...
def hurst_exponent(df: pd.DataFrame, window: int = 100, min_lags: int = 2, max_lags: int = 20) -> pd.Series:
    """
    ...
    log_prices = np.log(df['close'].replace(0, np.nan))
    ...
    var_1 = returns.rolling(window=window, min_periods=window//2).var()
    
    # Variance of lag-k returns
    k = max_lags
    returns_k = log_prices.diff(periods=k).fillna(0)
    var_k = returns_k.rolling(window=window, min_periods=window//2).var()
...
def statistical_divergence(df: pd.DataFrame, window: int = 14) -> pd.Series:
    ...
    lookback = 5
    price_min = df['close'].rolling(window=lookback).min()
    price_max = df['close'].rolling(window=lookback).max()
    rsi_min = rsi.rolling(window=lookback).min()
    rsi_max = rsi.rolling(window=lookback).max()

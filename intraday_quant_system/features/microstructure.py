...
def volume_synchronized_probability_of_informed_trading(
    ...
    delta_p = df["close"].shift(1).diff()
    sigma = (
        delta_p.shift(1).rolling(window=window, min_periods=5).std().replace(0, 1e-5)
    )
    ...
    if valid_mask.any():
        p_buy.loc[valid_mask] = norm.cdf(
            delta_p.loc[valid_mask] / sigma.loc[valid_mask]
        )
    ...
    # Shift by 1 to prevent lookahead bias
    rolling_v_buy = v_buy.shift(1).rolling(window=window, min_periods=window // 2).sum()
    rolling_v_sell = (
        v_sell.shift(1).rolling(window=window, min_periods=window // 2).sum()
    )
    rolling_v_total = (
        df["volume"].shift(1).rolling(window=window, min_periods=window // 2).sum()
    )
...
def trade_sign_correlation(df: pd.DataFrame, window: int = 20) -> pd.Series:
    ...
    # Shift by 1 to prevent lookahead bias
    trade_signs = (
        np.sign(df["close"].shift(1).diff()).replace(0, np.nan).ffill().fillna(1)
    )
    ...
    rolling_mean_x = trade_signs.shift(1).rolling(window=window).mean()
    rolling_mean_y = lag_1.shift(1).rolling(window=window).mean()

    rolling_cov = (
        ((trade_signs.shift(1) - rolling_mean_x) * (lag_1.shift(1) - rolling_mean_y))
        .rolling(window=window)
        .mean()
    )
    rolling_var_x = (
        ((trade_signs.shift(1) - rolling_mean_x) ** 2).rolling(window=window).mean()
    )
    rolling_var_y = (
        ((lag_1.shift(1) - rolling_mean_y) ** 2).rolling(window=window).mean()
    )

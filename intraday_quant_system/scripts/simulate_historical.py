...
def simulate(symbol: str, days: int):
    ...
    times = (
        ...
        if hasattr(features_df, "index")
        else np.arange(len(features_df))
    )
    ...
    labels = LGBMAlphaModel.make_labels(
        features_df, atr_mult_up=1.5, atr_mult_down=1.5, horizon_minutes=120
    )
    ...
    tabnet_probs = (
        tabnet_preds[:, 1] if tabnet_preds.shape[1] > 1 else tabnet_preds[:, 0]
    )
    ...
    for i in range(len(test_df)):
        ...
        score = ensemble.compute_score(
            lgbm_prob=prob_lgbm, tabnet_prob=prob_tabnet, meta_prob=0.65
        )
        ...
        if signal in ["buy", "sell"]:
            ...
            call = call_gen.generate_call(
                symbol, signal, c, a, score, "normal", 15.0, test_df.index[i]
            )
    ...
    if not df_trades.empty:
        ...
        logger.info(
            "No trades generated during test period. Filters were strictly active."
        )

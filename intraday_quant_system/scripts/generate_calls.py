...
from data.screener import FNO_UNIVERSE, DynamicScreener
    ...
    for symbol in symbols:
        ...
        try:
            ...
            df = engine.fetch_intraday_data(
                symbol, start_date, end_date, interval="15minute"
            )
            if not df.empty:
                all_data[symbol] = df
...
def run_pipeline(
    ...
    if features_df.empty or len(features_df) < 100:
        logger.warning(
            f"  {symbol}: Insufficient data ({len(features_df)} bars). Skipping."
        )
        return []
    ...
    if "volume" in features_df.columns:
        ...
        if len(recent_vol) >= bars_per_day:
            ...
            daily_vols = [
                recent_vol.iloc[i * bars_per_day : (i + 1) * bars_per_day].sum()
    ...
    if last_close < 50:
        logger.warning(
            f"  {symbol}: Price ₹{last_close:.2f} below ₹50 minimum. Skipping penny stock."
        )
        return []
    if avg_daily_volume < 200_000 and avg_daily_volume > 0:
        logger.warning(
            f"  {symbol}: Est. daily volume {avg_daily_volume:,.0f} below 200K minimum. Skipping illiquid stock."
        )
        return []
    ...
    for i in range(len(close)):
        if (
            i + horizon >= len(close)
            or np.isnan(atr_series.iloc[i])
            or atr_series.iloc[i] <= 0
        ):
    ...
    if len(labeled_df) < 100:
        logger.warning(
            f"  {symbol}: Only {len(labeled_df)} labeled bars after filtering neutrals. Skipping."
        )
        return []

    pos_rate = labeled_df["label"].mean()
    logger.info(
        f"  {symbol}: Label distribution — {pos_rate:.1%} bullish, {1-pos_rate:.1%} bearish ({len(labeled_df)} labeled bars)"
    )
    ...
    try:
        ...
        logger.warning(
            f"  {symbol}: TabNet training failed: {e}. Falling back to 0.5 prob."
        )
    ...
    try:
        ...
        logger.warning(
            f"  {symbol}: Meta-labeler training failed: {e}. Using default confidence."
        )
    ...
    if call_gen is None:
        ...
        call_gen = CallGenerator(
            min_risk_reward=1.2, min_confidence=0.40, no_new_calls_after="15:30"
        )

    # Get conformal threshold from meta-labeler
    conformal_threshold = (
        meta.conformal_threshold if hasattr(meta, "conformal_threshold") else 0.5
    )
    ...
    for i in range(start_idx, len(X_val)):
        ...
        if signal in ("buy", "sell"):
            ...
            current_atr = (
                val_df["atr"].iloc[i]
                if not np.isnan(val_df["atr"].iloc[i])
                else current_close * 0.015
            )

            # VPVR Support/Resistance
            support, resistance = get_closest_support_resistance(
                current_close, vpvr_data
            )
            ...
            top_features = [
                f"{'+' if shap_vals[idx] > 0 else '-'}{feature_names[idx]}"
                for idx in top_idx
            ]
...
def main():
    ...
    parser.add_argument(
        "--dynamic-screener",
        action="store_true",
        help="Use pre-market gap screener to pick top stocks dynamically",
    )
    parser.add_argument(
        "--top-n",
        type=int,
        default=15,
        help="Number of stocks to pick if using dynamic screener",
    )
    parser.add_argument(
        "--output",
        type=str,
        default="calls_log.csv",
        help="Output CSV file for call log",
    )
    ...
    print("  Minimum Risk:Reward = 1:1.5")
    print("=" * 60)
    ...
    if os.environ.get("FYERS_CLIENT_ID") and os.environ.get("FYERS_ACCESS_TOKEN"):
        ...
        logger.warning(
            "Fyers credentials missing. Will fall back to delayed yfinance data."
        )

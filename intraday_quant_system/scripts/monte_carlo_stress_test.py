...
def run_monte_carlo(
    ...
    """
    logger.info(
        f"Starting Monte Carlo Simulation ({n_simulations} permutations, {n_trades} trades each)"
    )
    logger.info(
        f"Metrics: Win Rate={win_rate:.1%}, Avg Win={avg_win:.2f}%, Avg Loss={avg_loss:.2f}%"
    )
    ...
    logger.info(
        f"  Risk of Ruin (100% Loss):   {ruin_count_100pct / n_simulations:.2%}"
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--win-rate", type=float, default=0.442, help="Empirical win rate (0-1)"
    )
    parser.add_argument(
        "--avg-win", type=float, default=1.85, help="Average percentage win"
    )
    parser.add_argument(
        "--avg-loss",
        type=float,
        default=-0.92,
        help="Average percentage loss (negative)",
    )
    parser.add_argument(
        "--sims", type=int, default=10000, help="Number of simulations to run"
    )

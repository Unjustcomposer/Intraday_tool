...
def main():
    ...
    parser.add_argument(
        "--days", type=int, default=120, help="Number of days of historical data"
    )
    ...
    for symbol in args.symbols:
        ...
        feature_cols = [
            c for c in feature_cols if c in features_df.columns and c != "target"
        ]
        ...
        if mean_auc > 0.55 and pct_above_chance > 70:
            logger.info(
                f"  ✅ SIGNAL DETECTED — Mean AUC {mean_auc:.3f} with {pct_above_chance:.0f}% consistency"
            )
        elif mean_auc > 0.52:
            logger.info(
                f"  ⚠️  WEAK SIGNAL — Mean AUC {mean_auc:.3f}, needs further validation"
            )
        else:
            logger.info(
                f"  ❌ NO SIGNAL — Mean AUC {mean_auc:.3f} indistinguishable from random"
            )

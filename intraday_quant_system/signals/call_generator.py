...
class TradeCall:
    ...
    target_2: float
    risk_reward: float
    confidence: float       # Meta-labeler probability (0-1)
    regime: str
    vix: float
    atr: float
    timestamp: datetime
    ...
    def suggested_allocation_pct(self) -> float:
        """Kelly Criterion allocation: W - ((1 - W) / R)"""
        if self.risk_reward <= 0:
            return 0.0
        # self.confidence is out of 100, we need it out of 1
        win_rate = self.confidence / 100.0
...
class CallGenerator:
    ...
    def __init__(self, min_risk_reward: float = 2.0, min_confidence: float = 0.60,
                 no_new_calls_after: str = "14:30", max_net_exposure: int = 3, max_total_calls: int = 5):
    ...
    def generate_call(self, symbol: str, signal: str, close: float, atr: float,
                      confidence: float, regime: str, vix: float,
                      vpvr_support: float = None, vpvr_resistance: float = None,
                      news_sentiment: float = 0.0, shap_features: str = "",
                      timestamp: datetime = None) -> Optional[TradeCall]:
        """
        ...
        current_time = timestamp.time() if hasattr(timestamp, 'time') else datetime.now().time()
        if current_time > self.no_new_calls_after:
            logger.info(f"[{symbol}] Signal rejected: past {self.no_new_calls_after} cutoff")
            return None
        ...
        if total_calls >= self.max_total_calls:
            logger.info(f"[{symbol}] Signal rejected: Max daily trades reached ({self.max_total_calls})")
            return None
            
        if signal == 'buy' and net_exposure >= self.max_net_exposure:
            logger.info(f"[{symbol}] BUY rejected: Max net long portfolio exposure reached (+{self.max_net_exposure})")
            return None
            
        if signal == 'sell' and net_exposure <= -self.max_net_exposure:
            logger.info(f"[{symbol}] SELL rejected: Max net short portfolio exposure reached (-{self.max_net_exposure})")
            return None
        ...
        if signal == 'buy' and news_sentiment < -0.3:
            logger.info(f"[{symbol}] BUY rejected: negative news sentiment ({news_sentiment})")
            return None
        if signal == 'sell' and news_sentiment > 0.3:
            logger.info(f"[{symbol}] SELL rejected: positive news sentiment ({news_sentiment})")
            return None
        ...
        if signal == 'buy' and news_sentiment > 0.3:
            ...
            logger.info(f"[{symbol}] BUY news boost! Adjusted threshold from {self.min_confidence} to {adjusted_min_confidence:.4f}")
        elif signal == 'sell' and news_sentiment < -0.3:
            boost = min(0.05, abs(news_sentiment) * 0.1)
            adjusted_min_confidence -= boost
            logger.info(f"[{symbol}] SELL news boost! Adjusted threshold from {self.min_confidence} to {adjusted_min_confidence:.4f}")
            
        # --- Filter 2: Confidence gate ---
        if confidence < adjusted_min_confidence:
            logger.info(f"[{symbol}] Signal rejected: confidence {confidence:.2f} < {adjusted_min_confidence:.4f} (Required)")
            return None
        ...
        if signal == 'buy':
            ...
            raw_t1 = (vpvr_resistance - 0.1 * atr) if vpvr_resistance else close + 1.5 * atr
            ...
            raw_sl = (vpvr_resistance + 0.2 * atr) if vpvr_resistance else close + 1.0 * atr
        ...
        if risk_reward < self.min_risk_reward:
            logger.info(f"[{symbol}] Signal rejected: R:R {risk_reward:.2f} < {self.min_risk_reward}")
            return None
            
        # --- Filter X: Minimum Profit Margin ---
        reward_pct = (reward / close) * 100
        if reward_pct < 0.75:
            logger.info(f"[{symbol}] Signal rejected: T1 margin {reward_pct:.2f}% < 0.75% (too thin for taxes/fees)")
            return None
            
        # --- Filter Y: Maximum Stop Loss (Fix #1) ---
        risk_pct_val = (risk / close) * 100
        if risk_pct_val > 1.5:
            logger.info(f"[{symbol}] Signal rejected: Stop Loss {risk_pct_val:.2f}% > 1.5% (risk too wide)")
            return None
        
        # --- Filter 5: R:R sanity cap (anything > 10:1 is likely data artifact) ---
        if risk_reward > 10.0:
            logger.warning(f"[{symbol}] R:R {risk_reward:.1f} capped to 10.0 (likely VPVR artifact)")
        ...
        if vix > 30:
            logger.info(f"[{symbol}] Signal rejected: VIX {vix:.1f} > 30 (extreme volatility)")
            return None

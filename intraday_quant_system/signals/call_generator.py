...
class CallGenerator:
    ...
    def generate_call(self, symbol: str, signal: str, close: float, atr: float,
        ...
        if current_time > self.no_new_calls_after:
            logger.debug(f"[{symbol}] Signal rejected: past {self.no_new_calls_after} cutoff")
            return None
        
        # --- Filter 2: Confidence gate ---
        if confidence < self.min_confidence:
            logger.debug(f"[{symbol}] Signal rejected: confidence {confidence:.2f} < {self.min_confidence}")
            return None
        ...
        if signal == 'buy':
            direction = 'BUY'
            stop_loss = round(vpvr_support if vpvr_support else close - 1.0 * atr, 2)
            target_1 = round(vpvr_resistance if vpvr_resistance else close + 1.5 * atr, 2)
            target_2 = round(close + 2.5 * atr, 2)
            
            # Ensure target is actually above entry, and SL below (VPVR might be weird)
            if target_1 <= close: target_1 = round(close + 1.5 * atr, 2)
            if stop_loss >= close: stop_loss = round(close - 1.0 * atr, 2)
        else:
            direction = 'SELL'
            stop_loss = round(vpvr_resistance if vpvr_resistance else close + 1.0 * atr, 2)
            target_1 = round(vpvr_support if vpvr_support else close - 1.5 * atr, 2)
            target_2 = round(close - 2.5 * atr, 2)
        ...
        if risk_reward < self.min_risk_reward:
            logger.debug(f"[{symbol}] Signal rejected: R:R {risk_reward:.2f} < {self.min_risk_reward}")
            return None
        
        # --- Filter 5: VIX extreme filter ---
        if vix > 30:

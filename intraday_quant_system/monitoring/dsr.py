...
This is critical for avoiding overfitting in backtesting where many
strategy configurations are tried and the best one is selected.
"""
...
class DSRResult:
    ...
    dsr: float                    # Deflated Sharpe Ratio (probability true SR > benchmark)
    psr: float                    # Probabilistic Sharpe Ratio (no multiple testing correction)
    sr_observed: float            # Observed Sharpe Ratio (annualized)
    se_sr_annual: float           # Standard error of annualized Sharpe
    expected_max_sr: float        # Expected max Sharpe under null (multiple testing)
    n_trials: int                 # Number of independent trials
    variance_of_sharpes: float    # Cross-sectional variance of Sharpe ratios
    benchmark_sharpe: float       # Benchmark Sharpe threshold
    annualization_factor: float   # Periods per year for annualization
    ...
    def to_dict(self) -> dict:
        return {
            ...
            "se_sr_annual": self.se_sr_annual,
...
def calculate_dsr(
    ...
    The DSR adjusts the Sharpe Ratio for:
    1. Non-normal returns (skewness, kurtosis) via Mertens (2002) SE correction
    2. Multiple testing (number of trials/configurations tried)
    3. Sample length (finite sample correction)
    ...
    Returns:
        DSRResult with DSR value (probability true Sharpe > benchmark after corrections)
        DSR >= 0.95 means 95% confidence strategy has positive edge after selection bias
    """
    if len(returns) < 2:
        return DSRResult(
            dsr=0.0, psr=0.0, sr_observed=0.0, se_sr_annual=0.0,
            expected_max_sr=0.0, n_trials=n_trials,
    ...
    se_sr = np.sqrt(
        (1 + 0.5 * sr_observed**2 - skew * sr_observed + 
         (kurt - 3) / 4 * sr_observed**2) / (n - 1)
    )
    ...
    if se_sr_annual == 0:
        return DSRResult(
            dsr=1.0 if sr_observed > benchmark_sharpe else 0.0,
            psr=1.0 if sr_observed > benchmark_sharpe else 0.0,
            sr_observed=sr_observed, se_sr_annual=0.0,
            expected_max_sr=0.0, n_trials=n_trials,
            variance_of_sharpes=variance_of_sharpes,
            benchmark_sharpe=benchmark_sharpe,
            annualization_factor=annualization_factor
    ...
    expected_max_sr = benchmark_sharpe + np.sqrt(variance_of_sharpes) * stats.norm.ppf(1 - 1/n_trials)
    ...
    return DSRResult(
        ...
        se_sr_annual=float(se_sr_annual),
...
def calculate_min_track_record(
    ...
    numerator = 1 + 0.5 * target_sr**2 - skew * target_sr + (kurt - 3) / 4 * target_sr**2

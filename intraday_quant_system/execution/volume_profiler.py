...
class VolumeProfiler:
    ...
    def compute_intraday_profile(
        df: pd.DataFrame, time_interval: str = "15min"
    ) -> list[float]:
        """
        ...
        if df.empty or "volume" not in df.columns:
            logger.warning(
                "Empty dataframe or no volume column. Returning flat profile."
            )
            return VolumeProfiler.flat_profile(
                25
            )  # Default 25 bins (375 mins / 15 mins)
        ...
        if sum_frac == 0 or np.isnan(sum_frac):
            return VolumeProfiler.flat_profile(
                len(avg_profile) if len(avg_profile) > 0 else 25
            )
        ...
        df.drop(
            columns=["_time", "_date", "_daily_total", "_vol_fraction"],
            inplace=True,
            errors="ignore",
        )

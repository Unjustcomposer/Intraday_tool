...
class PostTradeAnalyzer:
    ...
    def log_trade(
        ...
        logger.info(
            f"Logged trade for {symbol}: IS={is_bps:.1f} bps, Slippage={actual_slippage_bps:.1f} bps (vs Pred={pred_slippage_bps:.1f} bps), Fill={filled_status}"
        )
    ...
    def get_summary_stats(self) -> dict[str, Any]:
        ...
        fill_rates = {
            status: float(count / total_orders) for status, count in fill_counts.items()
        }
        ...
        stats = {
            ...
            "slippage_bias_bps": float(
                (df["actual_slippage_bps"] - df["predicted_slippage_bps"]).mean()
            ),
            "fill_rates": fill_rates,
        }

        logger.info(
            f"Post-Trade Performance Summary: {stats['total_trades']} trades, Mean IS: {stats['mean_is_bps']:.1f} bps, Mean Slip: {stats['mean_slippage_bps']:.1f} bps"
        )
        return stats

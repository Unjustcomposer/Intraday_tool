...
class ExecutionAlgorithm:
    ...
    def get_next_slice(
        self, current_time: datetime, market_data: dict[str, Any] = None
    ) -> int:
...
class VWAP(ExecutionAlgorithm):
    ...
    def get_next_slice(
        ...
        if (
            ...
            or self.current_interval >= self.total_intervals
        ):
            self.is_active = False
            remaining = self.total_quantity - self.executed_quantity
            if remaining > 0:
...
class AlmgrenChriss(ExecutionAlgorithm):
    ...
    def _calculate_trajectory(self):
        ...
        for i in range(1, self.intervals + 1):
            ...
            x_prev = (
                self.total_quantity
                * math.sinh(kappa * (T - t_prev))
                / math.sinh(kappa * T)
            )
            x_curr = (
                self.total_quantity * math.sinh(kappa * (T - t)) / math.sinh(kappa * T)
            )
    ...
    def get_next_slice(
        ...
        if (
            not self.is_active
            or current_time > self.end_time
            or self.current_interval >= self.intervals
        ):
            self.is_active = False
            remaining = self.total_quantity - self.executed_quantity
            if remaining > 0:

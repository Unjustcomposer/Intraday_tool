...
class NewsFetcher:
    ...
    def __init__(self, feeds: list[str] = None):
        ...
        self._article_cache: dict[str, dict] = (
            {}
        )  # keyed by headline hash to avoid duplicates

    def fetch_recent_news(
        self, hours_back: int = 24, symbols: list[str] = None
    ) -> list[dict[str, Any]]:
        """
            ...
            try:
                ...
                for entry in feed.entries:
                    ...
                    matched_symbol = self._match_symbol(
                        headline + " " + body, symbols_lower, symbols or []
                    )
    ...
    @staticmethod
    def _match_symbol(
        text: str, symbols_lower: list[str], symbols_original: list[str]
    ) -> str:
        """Match stock symbol mentioned in text. Returns empty string if no match."""
        text_lower = text.lower()
        for sym_lower, sym_orig in zip(symbols_lower, symbols_original, strict=False):

...
def fetch_all_nse_symbols() -> List[str]:
    ...
    try:
        ...
        if 'SYMBOL' in df.columns:
            ...
            logger.warning(f"NSE CSV did not contain 'SYMBOL' column. Columns: {list(df.columns)}")
            return []
...
class DynamicScreener:
    ...
    def _download_batch(self, symbols: List[str], batch_size: int = 100) -> pd.DataFrame:
        """
            ...
            try:
                ...
                if len(batch) == 1:
                    ...
                    if 'Close' in df.columns and 'Open' in df.columns:
                        ...
                        volumes = df[['Volume']].rename(columns={'Volume': ticker}) if 'Volume' in df.columns else None
        ...
        try:
            ...
            try:
                ...
                if not nifty.empty and 'Close' in nifty.columns:
                    ...
                    logger.info(f"NIFTY50 5-day baseline momentum: {nifty_momentum*100:+.2f}%")
            except Exception as e:
            ...
            if closes.empty or opens.empty:
                logger.error("Screener failed to download data. Falling back to F&O top-N.")
                    ...
                    if len(ticker_closes) >= 2 and len(ticker_opens) >= 1:
                        ...
                        if prev_close > 0:
                            ...
                            if avg_vol >= 100_000 and avg_turnover >= min_turnover:
                                gap_stocks.append((ticker, abs(gap_pct), gap_pct, avg_vol, last_close, 'gap'))
                            elif ticker in FNO_UNIVERSE:
                                gap_stocks.append((ticker, abs(gap_pct), gap_pct, avg_vol, last_close, 'fno_gap'))
                            else:
                                rejected_volume += 1
            ...
            logger.info(f"Gap scan: {rejected_price} rejected (price < ₹50), "
                ...
                try:
                    if ticker in closes.columns:
                        ...
                        if len(tc) >= 2:
                            ...
                            fno_momentum.append((ticker, relative_strength, 0.0, avg_vol, last_close, 'fno_momentum'))
                except Exception:
                    continue

            logger.info(f"F&O momentum scan: {len(fno_momentum)} additional F&O stocks by recent momentum")

            # Fix #8: Sector Grouping filter
            FNO_SECTORS = {
                "RELIANCE.NS": "Energy", "ONGC.NS": "Energy", "BPCL.NS": "Energy", "PETRONET.NS": "Energy",
                "HDFCBANK.NS": "Bank", "ICICIBANK.NS": "Bank", "SBIN.NS": "Bank", "AXISBANK.NS": "Bank", "KOTAKBANK.NS": "Bank",
                ...
                "BAJFINANCE.NS": "Fin", "CHOLAFIN.NS": "Fin",
            ...
            for candidate in all_candidates:
                ...
                if sector_counts.get(sector, 0) < max_per_sector or sector == "Other" or is_massive_gap:
            ...
            for ticker, sort_val, gap_pct, vol, price, source in selected:
                ...
                if source.startswith('gap') or source == 'fno_gap':
                    logger.info(f"  {ticker:<15} Gap: {gap_pct*100:+.2f}%  AvgVol: {vol_str}  Price: ₹{price:.1f}  [{source}]")
                else:
                    logger.info(f"  {ticker:<15} Mom: {sort_val*100:+.2f}%  AvgVol: {vol_str}  Price: ₹{price:.1f}  [{source}]")

...
class TickerProcess:
    ...
    async def _monitor_connection_async(self):
        ...
        while True:
            ...
            if latency > 2.0 and latency <= 30.0:
                logger.warning(
                    f"LATENCY WARNING: WebSocket latency={latency:.1f}s exceeds 2s threshold. Monitoring..."
                )

            # Kill-switch: disconnect or latency > 30 seconds
            if not self.is_connected or latency > 30.0:
                logger.critical(
                    f"HEARTBEAT FAILURE: WebSocket connected={self.is_connected} | Latency={latency:.1f}s exceeds 30s limit!"
                )
    ...
    def activate_kill_switch(self):
        ...
        logger.critical(
            "KILL-SWITCH TRIGGERED: Websocket disconnected for more than 30 seconds!"
        )
        try:
            ...
            if not positions.empty:
                logger.critical(
                    f"Kill-switch liquidating {len(positions)} open positions..."
                )
    ...
    async def _run_simulation_async(self):
        ...
        logger.info(
            "Async Simulation Mode active: Generating mock ticks for universe..."
        )
    ...
    async def run_async(self):
        ...
        if not self.kws:
            logger.info(
                "No Kite client configured. Switching to ASYNC SIMULATION MODE."
            )
        ...
        try:
            ...
            logger.warning(
                f"Real WebSocket connection failed: {e}. Switching to ASYNC SIMULATION MODE."
            )
            await self._run_simulation_async()

...
from intraday_quant_system.execution.post_trade_analyzer import PostTradeAnalyzer
...
from intraday_quant_system.models.tft_model import TemporalFusionTransformerModel
from intraday_quant_system.models.xgboost_model import XGBoostAlphaModel
from intraday_quant_system.monitoring.drift_detector import FeatureDriftMonitor, SignalDecayMonitor
from intraday_quant_system.monitoring.grafana_exporter import GrafanaExporter
from intraday_quant_system.nlp.finbert_sentiment import SentimentEngine
from intraday_quant_system.regime.hmm_regime import RegimeDetector
from intraday_quant_system.risk.portfolio_risk import PortfolioRiskMonitor
from intraday_quant_system.risk.stop_loss import VolatilityStopEngine
from intraday_quant_system.signals.ensemble import EnsembleScorer
from intraday_quant_system.data.market_data import DataStorage, MarketDataEngine
from intraday_quant_system.data.news_data import NewsFetcher
from intraday_quant_system.data.nse_calendar import NSECalendar
...
class PipelineRunner:
    ...
    def __init__(self, config_path: str = "config.yaml"):
        ...
        self.storage = DataStorage(
            timescaledb_url=self.config.timescaledb_url, redis_url=self.config.redis_url
        )
        self.news_fetcher = NewsFetcher()
        self.feature_store = FeatureStore(
            bars_per_day=self.config.intraday.bars_per_day
        )

        # Models
        self.regime_detector = RegimeDetector()
        self.alpha_model = LGBMAlphaModel(
            config={"num_leaves": self.config.models.lgbm.num_leaves}
        )
        ...
        self._sentiment_executor = ThreadPoolExecutor(
            max_workers=1, thread_name_prefix="finbert"
        )
        ...
        self.order_manager = OrderManager(
            ...
            self.stop_engine,
            config={
                "intraday": self.config.intraday.__dict__,
                "risk": self.config.risk.__dict__,
            },
        )
    ...
    def _load_models(self):
        ...
        try:
            ...
            logger.error(
                f"Failed to load models: {e}. Will run with default/untrained behavior."
            )
    ...
    def setup_schedules(self):
        ...
        self.scheduler.add_job(
            ...
            CronTrigger(
                day_of_week="mon-fri", hour=9, minute=0, timezone="Asia/Kolkata"
            ),
        )
        ...
        self.scheduler.add_job(
            ...
            CronTrigger(
                day_of_week="mon-fri", hour=15, minute="15-30", timezone="Asia/Kolkata"
            ),
        )
        ...
        if not self._killswitch_running:
            ...
            self._killswitch_thread = threading.Thread(
                target=self._news_killswitch_loop, daemon=True
            )
        ...
        self.scheduler.add_job(
            ...
            CronTrigger(
                day_of_week="mon-fri", hour=15, minute=45, timezone="Asia/Kolkata"
            ),
        )
    ...
    def market_open_routine(self):
        ...
        if not self.nse_calendar.is_trading_day(ist_now):
            logger.info(
                f"{ist_now.date()} is a market holiday. Skipping market open prep."
            )
            return
        ...
        try:
            ...
            for sym in self.config.universe:
                ...
                df = self.market_data.fetch_historical_data(
                    sym, start_time, ist_now, interval="5minute"
                )
                if not df.empty:
            ...
            articles = self.news_fetcher.fetch_recent_news(
                hours_back=18, symbols=self.config.universe
            )
            self.news_fetcher.store_news(articles)

            for sym in self.config.universe:
                sym_articles = [
                    a for a in articles if a.get("symbol", "").upper() == sym.upper()
                ]
                if sym_articles and self.sentiment_engine.is_ready:
                    res = self.sentiment_engine.analyze(
                        sym_articles[0].get("headline", "")
                    )
    ...
    def trigger_retraining(self):
        ...
        logger.critical(
            "PSI drift threshold breached! Triggering online incremental model retraining..."
        )
        self.log_audit_event(
            "RETRAIN_TRIGGERED", {"reason": "PSI drift threshold exceeded"}
        )
        try:
            # Reload/refresh model weights
            self._load_models()
            logger.info(
                "Online model retraining completed. Refreshed model weights successfully."
            )
            ...
            try:
                ...
                for article in articles:
                    ...
                    if any(kw in headline for kw in fatal_keywords):
                        logger.critical(
                            f"BLACK SWAN EVENT DETECTED: {headline}. Halting trading immediately!"
                        )
                        self.risk_monitor.trading_halted = True
                        self.log_audit_event(
                            "NEWS_KILLSWITCH_TRIGGERED", {"headline": headline}
                        )
                        self.enforce_hard_exit()
                        break
            except Exception as e:
    ...
    def main_loop(self):
        ...
        for message in self.pubsub.listen():
            ...
            try:
                ...
                end_time = pd.to_datetime(bar_data["timestamp"]).tz_convert(
                    "Asia/Kolkata"
                )
                ...
                if symbol not in self._market_cache or self._market_cache[symbol].empty:
                    ...
                    df = self.market_data.fetch_historical_data(
                        symbol, start_time, end_time, interval="5minute"
                    )
                ...
                if self.drift_monitor.baseline_stats:
                    ...
                    # Z-score check
                    severe_drift = sum(
                        1 for v in drift_result["z_scores"].values() if v > 3.0
                    )
                    if severe_drift >= 3:
                        self._drift_halt_counter += 1
                        logger.warning(
                            f"Severe drift in {severe_drift} features (bar {self._drift_halt_counter}/3)"
                        )
                        if self._drift_halt_counter >= 3:
                            logger.critical(
                                "DRIFT AUTO-HALT: 3 consecutive bars with severe drift. Halting trading."
                            )
                            self.risk_monitor.trading_halted = True
                            self.log_audit_event(
                                "DRIFT_AUTO_HALT",
                                {
                                    "reason": "3 consecutive bars with severe Z-score drift"
                                },
                            )
                            continue
                    else:
                    ...
                    if drift_result["needs_retrain"]:
                        logger.warning(
                            "Online drift detection triggered! Initializing incremental model retrain."
                        )
                    ...
                    try:
                        ...
                        if len(features_df) >= self.tft_model.sequence_length:
                            seq_data = (
                                ...
                                .iloc[-self.tft_model.sequence_length :]
                                .values
                            )
                ...
                meta_prob = self.meta_labeler.predict_proba(
                    latest_features, np.array([lgbm_prob])
                )[0]
                ...
                self.exporter.update_all(
                    {
                        "quant_daily_pnl": float(
                            status["daily_pnl_pct"] * self.config.max_capital
                        ),
                        "quant_open_positions": float(
                            len(positions) if not positions.empty else 0.0
                        ),
                        "quant_ensemble_score": float(score),
                        "quant_drawdown": float(status["drawdown"]),
                        "quant_regime": float(encoded_regime),
                        "quant_vix": float(vix),
                        "quant_signal_auc": float(
                            self.signal_monitor.latest_auc
                        ),  # Actual computed rolling AUC (0.0 if not yet computed)
                    }
                )
    ...
    def _schedule_sentiment_update(self, symbol: str):
        ...
        def _fetch_and_analyze():
            try:
                articles = self.news_fetcher.fetch_recent_news(
                    hours_back=2, symbols=[symbol]
                )
                symbol_articles = [
                    a for a in articles if a.get("symbol", "").upper() == symbol.upper()
                ]
            ...
            try:
                ...
                if not positions.empty:
                    ...
                    for sym in positions["symbol"]:
                        ...
                        if (
                            sym in self._market_cache
                            and not self._market_cache[sym].empty
                        ):
                            current_prices[sym] = self._market_cache[sym]["close"].iloc[
                                -1
                            ]
                        else:
                            df = self.market_data.fetch_historical_data(
                                sym, ist_now - pd.Timedelta(days=1), ist_now, "5minute"
                            )
                            if not df.empty:
    ...
    def end_of_day_routine(self):
        ...
        try:
            ...
            status = self.risk_monitor.get_status()
            logger.info(
                f"Daily PnL: {status['daily_pnl_pct']:.2%} | Current Capital: {status['capital']:.2f}"
            )

...
class ConformalPredictor:
    ...
    def calibrate(self, cal_probs: np.ndarray, cal_labels: np.ndarray):
        ...
        scores_0 = cal_probs[
            cal_labels == 0
        ]  # P(1 | x) is the wrong probability for true label 0
        scores_1 = (
            1.0 - cal_probs[cal_labels == 1]
        )  # 1 - P(1 | x) is the wrong probability for true label 1

        self.cal_scores_class0 = np.sort(scores_0)
        self.cal_scores_class1 = np.sort(scores_1)
        self.is_calibrated = (
            len(self.cal_scores_class0) > 0 and len(self.cal_scores_class1) > 0
        )
    ...
    def predict_set(
        self, test_probs: np.ndarray, significance_level: float = 0.05
    ) -> list[set[int]]:
        """
    ...
    def get_signal_confidence(
        self, test_prob: float, significance_level: float = 0.05
    ) -> tuple[str, float]:
        """
